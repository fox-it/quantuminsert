...
def main():
    parser = OptionParser()
    parser.add_option("-s", "--shooter", default=None,
                        help="ip of the shooter server")
    parser.add_option("-p", "--port", type=int, default=1111,
                        help="(udp) port of the shooter")
    parser.add_option("-t", "--tshark", default=False,
                        action="store_true",
    ...
    while True:
        ...
        print tip._asdict().values()
        data = struct.pack(TIP_STRUCT, *[tip.src, tip.dst, tip.sport, tip.dport, tip.seq, tip.ack])
